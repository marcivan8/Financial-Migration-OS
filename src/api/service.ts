...
export class MigrationService {
   ...
   * in memory, without writing anything. This is the same
   * validate → read → plan → simulate sequence `createMigration` runs
   * inline below, pulled out so `BatchPipeline.planBatch` can gather many
   * customers' worth of it — reads run per customer either way, nothing
   * here batches those — before writing any of it through `persistPrepared`.
   *
   * `createMigration` itself is left as its own inline implementation
   * rather than rebuilt on top of this: the two would need to reconcile
   * `persist()`'s live-`Migration`-instance signature (which `authorize`,
   * `advanceTask`, `simulate` and `resolveException` all still need as-is)
   * with a plain `PreparedMigration` snapshot, and that reconciliation buys
   * nothing the extensive regression coverage on the single-migration path
   * is worth risking for.
   *
   * Returns `existing` when the idempotency key already resolved to a
   * migration — nothing to plan or persist, the caller already has it.
   */
